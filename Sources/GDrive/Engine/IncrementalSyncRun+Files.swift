...
extension IncrementalSyncRun {
    ...
    func scheduleFiles(_ fileItems: [DirtyRecord], duringScan: Bool) async throws {
        ...
        for item in fileItems {
            ...
            if duringScan && !directoryContext.isReady(item.parentId) { continue }
                ...
                Task {
                    ...
                    do {
                        ...
                        if fSize > 8 * 1024 * 1024 {
                            ...
                            uploadedFile = try await engine.performResumableUpload(
                                rootId: rootID,
                            ...
                            uploadedFile = try await engine.client.uploadMultipart(
                                name: item.name,
                            ...
                            uploadedFile = try await engine.client.updateMultipart(
                    ...
                    do {
                        ...
                        let original = rootURL.appendingPathComponent(parentRel)
                            .appendingPathComponent(
                                item.name)
                        let op = try await ConflictOperation.prepare(
                            store: engine.store,
