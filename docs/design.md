...
* **`SyncEngineAPI.swift`**：
  ...
  * **下载暂存目录**：通过初始化参数 `downloadTemporaryDirectory` 或
    ...
    原子交换的恢复文件也留在该处，避免被增量扫描上传。同步退出时用 `rmdir` 删除空的根暂存目录，
    非空目录保留。暂存与目标需位于同一文件系统；
...
引擎不保存运行对象。执行顺序为根校验与冲突恢复、Changes 消费、流式扫描与提前传输、
缺失登记、目录创建恢复、最终文件调度、目录处理、远端补列和持久化收尾。
扫描失败或取消时先等待已启动传输完成再抛错，不进入缺失检测；目录删除在文件任务及其回执完成后执行。

本地文件监听通过 `notifyLocalChanges(_:)` 提交 `LocalChange`，包含新增、修改、删除、
移动及文件/目录类型，移动携带旧、新路径。通知按本地路径解析 SQLite 根绑定或当前运行的
父目录同步任务，接收完成即返回。显式同步的数据库连通性检查通过并取得根锁后，在核心同步
开始前登记活动根路径，使初始化尚未写入根记录时也能接收通知。

`RootSyncCoordinator` 为每个本地根维护一个运行任务和一个合并中的 pendingTask。当前
整轮同步结束（包括扫描、传输及回执）后原子取出并清空 pendingTask，再执行下一轮；
空闲根收到通知时同样创建 pendingTask，并立即启动。期间的新通知形成新的 pendingTask，
逐轮串行执行直到清空。不同根以及单轮内部传输仍可并行。
changes 只触发一次本地观察，失败后记录日志，不在内存中保留或重放；期间收到的新 changes
仍正常执行。`pendingLocalChangeStatus()` 只报告运行状态及尚未消费的通知数量。
pendingTask 执行及批次交接期间持续占用根锁，同根显式同步调用返回 `rootBusy`。
队列只在进程内维护，重启通过显式整根同步重新观察，不新增调度锁或队列的 SQL 表。

pendingTask 执行前重新观察当前本地状态与基线，丢弃过时操作；通知种类不是直接执行远端
修改的指令。局部观察的文件重新核实 SHA-256，目录扫描当前子树，移动覆盖旧、新位置。
局部缺失检测和 dirty 调度限制在对应范围内，未扫描条目不能视为删除。文件内容观察和
缺失状态先提交 SQLite，再执行传输；未完成的差异保留在 items 的本地观察、dirty 状态及
既有 operations 中，由后续同步继续处理，不依赖 watcher 事件重放。执行时找不到根绑定则
记录 warning 并丢弃该根的本批通知；数据库读取错误仍作为错误报告。

全量和局部扫描都只接受已映射且已就绪的父目录，不把缺失父映射解释为同步根。既有
...
结果的数据库错误会终止本轮；能够在条目级隔离的数据库错误只记录 error log。watcher
后台任务在边界记录错误并丢弃该批 changes，不提供额外错误事件或内存重试。显式同步无法
...
* **模式 3：`syncIncremental`**：
  ...
  * 初始化和增量产生的内容冲突统一写入 `sync_conflicts`。未解决期间该相对路径被同步门禁
    ...
    一侧删除而另一侧仍为基线内容时生成删除意图；本地删除先按扫描版本和 SHA-256 核验，原子隔离后
    再次核验，且 SQLite 回执必须匹配三项 generation。目录列举失败或存在任何条目（包括隐藏项）时
    阻断删除。Google Drive 条件元数据 PATCH 尚未通过真实契约验证，因此自动远端删除保持 blocked，
    不以 GET 后无条件 PATCH 代替原子前置条件。一侧删除而另一侧已修改时写入冲突表。
    调用方选择被删除的一侧会把删除扩散到另一侧，选择已修改的一侧会恢复被删除的一侧。
...
* **`BootstrapTaskRegistry.swift`**：
  ...
  * 扫描失败或取消会取消已登记任务，并等待全部任务收尾后抛错；取消开始后登记的任务
    也会被取消。为支持取消和排空，本轮结束前保留所有已登记任务句柄，空间随条目数量增长。
    初始化上传不再使用 `DirectoryTracker` 的等待表或单独的本地目录 ID 表。
