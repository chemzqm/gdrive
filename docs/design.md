...
* **`SyncEngineAPI.swift`**：
  ...
  * **运行状态隔离**：所有公开同步入口在远端请求和本地扫描前解析符号链接与缺失路径祖先，
    对当前目录和 SQLite 中的全部绑定根检查认证文件、SQLite 数据库、下载暂存目录和冲突目录，
...
| `PathOperation.swift` | 文件与目录共用的路径操作：远端重命名/移动及条件回执；尚未落地且旧路径不存在的文件直接更新映射，已有本地对象在排他 rename 前后以 no-follow 方式校验类型和 device/inode，结果身份不符时回滚，中断重放只接受同身份目标 |
...
* **内容冲突**：初始化和增量流程都把冲突写入 `sync_conflicts`，保留本地内容，并将已校验的
  ...
  冲突记录及其远端副本并递增 revision，不存在后台自动解决流程。解决操作只在用冲突 ID
  找到根并取得根锁后重读当前快照；数据库回执同时校验 conflict revision 与 item generations，
  旧快照不能覆盖同步轮次刷新后的状态。子文件路径按当前 items 目录树解析；无法解析时忽略并保留冲突记录。
  其余情况下，解决接口正常返回时冲突记录已删除；若无法完成则抛错并保留冲突。
...
* **模式 2：`syncRemoteToLocalEmpty`**：
  * 流式递归列举远端目录树，边扫描边下载并流式计算 SHA-256，临时文件下载完成后
    通过目标原路径分块写入落盘，使文件监听收到内容变化事件。远端元数据缺少 SHA-256 时，
    基线和冲突回执使用下载时计算并校验的值，不以空哈希提交完成状态；初始化重试从已提交
    基线恢复该摘要，校验已落地文件后清除重新枚举产生的观察。
...
* **模式 3：`syncIncremental`**：
  ...
  * 初始化和增量产生的内容冲突统一写入 `sync_conflicts`。未解决期间该相对路径被同步门禁
    ...
    选择 `.local` 时，若旧远端文件仍存在，先按其 ID 移入 Drive 废纸篓；成功后在同一事务中
    删除所选文件的旧数据库基线和冲突记录，保留本地文件。旧远端已删除时直接清理旧记录。
    下次扫描将保留的本地文件作为新文件上传，分配新的远端 ID；trash 或数据库回执失败则抛错并保留冲突。
    本地文件已删除而远端正文已修改时，选择 `.remote` 会以独占创建方式重新生成本地文件；
    若创建前同路径重新出现文件，保留冲突等待再次处理。
    一侧删除而另一侧仍为基线内容时生成删除意图。本地文件响应远端删除前与 SQLite 基线比较一次；
    若已变化，直接更新本地 SHA-256 和文件身份并写入远端已删除的 `sync_conflicts`。若匹配，移入废纸篓；
    系统返回移走后的路径时再计算一次 SHA-256，未返回路径时记录错误日志并按移走成功处理。
    ...
    记录独立于随后删除的 items 子树，
    只保存 `root_id`、原文件全路径、保管文件全路径和记录时间。`listConflicts(localPath:)`
    将其与内容冲突一起列出；`resolveConflict(id:resolution:)` 选择 `.local` 时将保管文件
    恢复到原路径，选择 `.remote` 时将保管文件移入系统废纸篓，成功后删除对应行。
...
* **`LocalBaselineCache.swift`**：
  ...
  * 初始化上传仅在父目录已就绪，且 `parent_id`、文件名和五项元数据全部匹配时跳过，
    防止共享 inode 的硬链接或不同路径误用其他条目的基线。
  * 路径身份已确认且五项元数据完全一致时，**不进行任何文件内容读取与 SHA-256 哈希计算**，在纳秒
    级秒级跳过数万个未修改文件。
* **`Reconciler.swift`**：
