...
struct SyncEngineTests {
    ...
    func testLocalToRemoteEmptyRoundTrip() async throws {
        ...
        try await withRemoteTestDirectoryCleanup(client: client, remoteID: remoteRootID) {
            let remoteRoot = try await client.createDirectory(name: remoteTestDirName, parentId: rootID, remoteId: remoteRootID)
    ...
    func testFileAndDirectoryLifecycleEvents() async throws {
        ...
        try await withRemoteTestDirectoryCleanup(client: client, remoteID: remoteRootID) {
            ...
            var remoteChildren: [DriveFile] = []
            for _ in 0..<20 {
                remoteChildren = try await client.listChildren(parentId: remoteRoot.id)
                let names = remoteChildren.map(\.name)
                if names.contains("folder_renamed") && !names.contains("folder1") { break }
            ...
            var deleteStats = SyncStats()
            for _ in 0..<20 {
                deleteStats = try await engine.syncIncremental(localPath: tempDir.path)
                if deleteStats.filesDeleted > 0 { break }
                try await Task.sleep(for: .milliseconds(200))
            }
    ...
    func testLargeFileResumeInvalidatedWhenFileModified() async throws {
        ...
        try await withRemoteTestDirectoryCleanup(client: client, remoteID: remoteRootID) {
            ...
            let testStore = try await StateStore(path: testDbPath)
            let retainedRemoteID = try #require(remoteRootGenIds.dropFirst().first)
